...
void main() {
  ...
  group('legacy memory mode (tool handler)', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(names, [...MemoryTools.legacyToolNames, 'get_tool_result']);
    ...
    testWidgets('legacy tool descriptions follow the prompt language', (
      ...
      List<String> descriptionsFor(String lang) {
        ...
        return service
            ...
            .where(
              (d) => MemoryTools.legacyToolNames.contains(
                (d['function'] as Map)['name'],
              ),
            )
  ...
  group('memory_read', () {
    ...
    test('APK assistant excludes legacy per-app workflow records', () async {
      await memoryRepository.create(
        scope: MemoryScope.global,
        type: MemoryType.workflow,
        content: '此刻日记 APK 修改补丁已验证，签名成品为 diary_signed.apk。',
        source: MemorySource.tool,
      );
      await memoryRepository.create(
        scope: MemoryScope.global,
        type: MemoryType.instruction,
        content: '用户要求每次提问使用提问工具。',
        source: MemorySource.manual,
      );
      ...
      final raw = await call(
        ...
        const {},
        a: assistant(id: 'builtin-apk-mod'),
      );
      final payload = decode(raw!);

      expect(payload['total'], 1);
      expect((payload['entries'] as List).single['type'], 'instruction');
    });
  });
  ...
  group('memory_update', () {
    ...
    test('APK assistant rejects project writes to generic workflow', () async {
      final raw = await call(MemoryTools.memoryUpdate, {
        'type': 'workflow',
        'content': '来音制谱 APK 修改补丁已完成，等待签名成品验证。',
      }, a: assistant(id: 'builtin-apk-mod'));
      expect(decode(raw!)['error'], 'apk_project_memory_is_scoped');
    });
  });
