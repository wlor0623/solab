...
import 'package:solab/core/services/memory/memory_smart_add.dart';
...
''';
  ...
  group('applyDecision NEW/MERGE/CONFLICT/SKIP', () {
    ...
    test('without LLM merges the same APP workflow into one entry', () async {
      await seedAssistant('a1');
      final existing = await memoryRepository.create(
        scope: MemoryScope.global,
        type: MemoryType.workflow,
        content: 'Kelive APP 已完成 APK 工具接入。',
        source: MemorySource.manual,
      );
      final r = await smartAdd.addOne(
        item: const SmartAddItem(
          type: MemoryType.workflow,
          content: 'Kelive APP 已验证 Blutter 补丁字节。',
          scope: MemoryScope.global,
        ),
        visibilityAssistantId: 'a1',
        source: MemorySource.tool,
      ...
      expect(r.action, SmartAddAction.merge);
      expect(r.id, existing.id);
      expect(
        (await memoryRepository.readByType(MemoryType.workflow)),
        hasLength(1),
      );
    });
