...
void main() {
  ...
  test('concurrent instruction initialization seeds one default', () async {
    ...
    // 并发初始化只种子一次：环境默认 1 条 + APK Mod 种子 3 条 = 4 条，不重复追加。
    expect(provider.items, hasLength(4));
    expect(provider.items.first.prompt, isNotEmpty);
    ...
    expect(
      restored.items.map((e) => e.id).toSet(),
  ...
  test(
    ...
    () async {
      ...
      expect(
        provider.servers.where((server) => server.id == 'solab_fetch'),
        hasLength(1),
      );
      ...
      expect(
        restored.servers.where((server) => server.id == 'solab_fetch'),
        hasLength(1),
      );
  ...
  test(
    ...
    () async {
      ...
      expect(provider.getById('solab_fetch')?.name, '@kelivo/fetch');
      ...
      expect(restored.getById('solab_fetch')?.name, '@kelivo/fetch');
      await reopened.close();
    },
  );
}
