...
void main() {
  ...
  group('SettingsProvider reasoning support', () {
    ...
    test('Agnes, Ox Alpha, MiMo and Hy3 expose documented controls', () {
      final cases = <String, bool>{
        'agnes-2.5-flash': true,
        'agnes-2.5-pro': true,
        'agnes-2.5-pro-alpha': true,
        ...
        'minimax-m-2-7': false,
        'kimi-k-2-7-code': true,
      };

      for (final entry in cases.entries) {
        final model = ModelRegistry.infer(
          ModelInfo(id: entry.key, displayName: entry.key),
        );
        expect(model.abilities, contains(ModelAbility.tool), reason: entry.key);
        expect(
          model.abilities,
    ...
    test('logical model aliases use the upstream model capabilities', () {
      final cfg = ProviderConfig(
        ...
        models: const <String>['fast-model'],
        modelOverrides: const <String, dynamic>{
          'fast-model': <String, dynamic>{'apiModelId': 'agnes-2.5-flash'},
        },
      );

      final model = effectiveModelInfo(cfg, 'fast-model');
      expect(model.input, contains(Modality.image));
      expect(
        model.abilities,
        containsAll([ModelAbility.tool, ModelAbility.reasoning]),
    ...
    test('reasoning defaults to automatic on a fresh install', () async {
