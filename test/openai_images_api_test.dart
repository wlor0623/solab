...
void main() {
  group('OpenAI Images API', () {
    ...
    test('routes Agnes image models to generations', () async {
      ...
      final chunks = await ChatApiService.sendMessageStream(
        ...
        extraBody: const {'response_format': 'url'},
        stream: false,
      ).toList();
      ...
      expect(
        (requestBody['extra_body'] as Map)['response_format'],
        'url',
      );
      expect(chunks.firstImageUri, contains('agnes-generated.png'));
    });

    test('routes Agnes image edits through JSON generations', () async {
      late Uri requestUri;
      late String contentType;
      late Map<String, dynamic> requestBody;
      final tempDir = await Directory.systemTemp.createTemp(
        'kelivo_agnes_image_edit_',
      );
      addTearDown(() async {
      ...
      expect(requestBody['size'], '1024x1024');
      final nested = requestBody['extra_body'] as Map;
      expect(nested['image'], ['AQIDBA==']);
      expect(nested['response_format'], 'b64_json');
      expect(chunks.firstImageUri, contains('agnes-edited.png'));
    });
    ...
    test('saves base64 image responses with requested output format', () async {
      ...
      expect(imageUri, startsWith('solab-file:///'));
  ...
  group('OpenAI Responses image generation', () {
    ...
    test('emits Image events from non-stream output array', () async {
      ...
      expect(uri, startsWith('solab-file:///'));
