...
abstract final class MemoryTools {
  ...
  static Future<String> _handleMemoryRead({
    ...
    final all = assistant.id == 'builtin-apk-mod'
        ? visible.where((entry) => !_isLegacyApkWorkflow(entry)).toList()
        : visible;
  ...
  static Future<String> _handleMemoryUpdate({
    ...
    if (assistant.id == 'builtin-apk-mod' &&
        type == MemoryType.workflow &&
        _looksLikeApkProjectContent(content)) {
      return toolError(
        error: 'apk_project_memory_is_scoped',
        message:
            'APK 修改记录不能写入通用 workflow。请在当前 APK 报告下预存，等用户安装确认后再用专用验证工具合并为该软件唯一记录。',
        tool: memoryUpdate,
        instruction:
            'Use save_apk_patch_memory only after producing the signed artifact, then record_apk_patch_verification after explicit user validation.',
      );
    }
  ...
  static Future<String> _handleMemorySearchProfile({
    ...
    final rawMatched = await chatRepository.searchMemories(
      assistantId: assistant.id,
      tokens: tokens,
      type: type,
      matchAll: true,
    ...
    final matched = assistant.id == 'builtin-apk-mod'
        ? rawMatched.where((entry) => !_isLegacyApkWorkflow(entry)).toList()
        : rawMatched;
    ...
    final relatedFiltered = relatedEntries
        .where(
          (e) =>
              ...
              (assistant.id != 'builtin-apk-mod' || !_isLegacyApkWorkflow(e)) &&
  ...
  static bool _isLegacyApkWorkflow(MemoryEntry entry) =>
      entry.type == MemoryType.workflow &&
      _looksLikeApkProjectContent(entry.content);

  static bool _looksLikeApkProjectContent(String content) {
    final lower = content.toLowerCase();
    final apkIdentity =
        lower.contains('apk') ||
        lower.contains('安装包') ||
        lower.contains('libapp.so') ||
        lower.contains('_signed.apk') || lower.contains('_成品.apk');
    final modification =
        lower.contains('修改') ||
        lower.contains('补丁') ||
        lower.contains('已验证') ||
        ...
        lower.contains('签名成品');
    return apkIdentity && modification;
  }
  ...
  static Map<String, dynamic> _defMemoryRead(MemoryPromptLang lang) {
    ...
    return {
      ...
      'function': {
        'name': memoryRead,
        'description': zh
            ? '读取用户的通用长期记忆。type 可选：identity、workflow、voice、instruction。不传 type 则返回全部类型。APK 软件的修改产物和验证记录不在这里返回，必须先读取当前 APK 报告，再用 get_apk_patch_memory 按包指纹读取。'
            : 'Read generic long-term memory. Optional type: identity, workflow, voice, instruction. APK modification artifacts and validation records are excluded; read the current APK report first, then use get_apk_patch_memory for that package fingerprint.',
        'parameters': {
          'type': 'object',
          'properties': {
            'type': {
