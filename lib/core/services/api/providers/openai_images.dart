...
bool _isAgnesImageModel(String modelId) =>
    modelId.toLowerCase().startsWith('agnes-image-');
...
Stream<StreamChunk> sendOpenAIImagesStream(
  ...
  final isAgnes = _isAgnesImageModel(upstreamModelId);
  ...
  final response = input.imageRefs.isEmpty
      ...
      : isAgnes
      ? await _sendAgnesImageGeneration(
          client,
          config,
          modelId,
          input.prompt,
...
Future<Map<String, dynamic>> _sendOpenAIImageGeneration(
  ...
  if (_isAgnesImageModel(apiModelId(config, modelId))) {
    _normalizeAgnesImageBody(body);
  }
...
Future<Map<String, dynamic>> _sendAgnesImageGeneration(
  http.Client client,
  ProviderConfig config,
  String modelId,
  String prompt,
  ...
  final inputs = <({bool remote, String value, String dataUri})>[];
  for (final ref in imageRefs) {
    inputs.add(await _agnesImageInput(ref));
  }
  final body = <String, dynamic>{
    'model': apiModelId(config, modelId),
    'prompt': prompt,
  ...
  final nested = Map<String, dynamic>.from(
    body['extra_body'] as Map? ?? const <String, dynamic>{},
  );
  if (inputs.every((input) => input.remote)) {
    final urls = inputs.map((input) => input.value).toList(growable: false);
    body['image'] = urls.length == 1 ? urls.single : urls;
  } else if (inputs.every((input) => !input.remote)) {
    nested['image'] = inputs
        .map((input) => input.value)
        .toList(growable: false);
    nested.putIfAbsent('response_format', () => 'b64_json');
  } else {
    body['image'] = inputs
        .map((input) => input.remote ? input.value : input.dataUri)
        .toList(growable: false);
    nested.putIfAbsent('response_format', () => 'b64_json');
  }
  if (nested.isNotEmpty) body['extra_body'] = nested;
  final response = await client.post(
    _openAIImagesUrl(config, '/images/generations'),
    headers: _openAIImagesJsonHeaders(
      config,
...
Future<({bool remote, String value, String dataUri})> _agnesImageInput(
  ImageRef ref,
) async {
  if (ref.kind == 'url') {
    return (remote: true, value: ref.src, dataUri: ref.src);
  }
  if (ref.kind == 'data') {
    final comma = ref.src.indexOf(',');
    final value = (comma >= 0 ? ref.src.substring(comma + 1) : ref.src)
        .replaceAll(RegExp(r'\s'), '');
    final mime = ref.mime?.trim().isNotEmpty == true
        ? ref.mime!.trim()
  ...
  final path = SandboxPathResolver.fix(ref.src);
  final file = File(path);
  if (!await file.exists()) {
    throw FileSystemException('Agnes input image does not exist.', path);
  }
  final value = base64Encode(await file.readAsBytes());
  final mime = ref.mime?.trim().isNotEmpty == true
      ? ref.mime!.trim()
      : mimeFromPath(path);
  return (
...
void _normalizeAgnesImageBody(Map<String, dynamic> body) {
  body.putIfAbsent('size', () => '1024x1024');
  final responseFormat = body.remove('response_format');
  final rawNested = body['extra_body'];
  if (rawNested != null && rawNested is! Map) {
    throw const FormatException('Agnes extra_body must be an object.');
  }
  final nested = Map<String, dynamic>.from(
    rawNested as Map? ?? const <String, dynamic>{},
  );
  if (responseFormat != null) {
    nested.putIfAbsent('response_format', () => responseFormat);
  }
  if (nested.isNotEmpty) body['extra_body'] = nested;
}
...
Future<List<({String uri, String mimeType})>> _openAIImagesFromResponse(
  ...
  for (final item in data) {
    ...
    var b64 = (item['b64_json'] ?? '').toString().trim();
    if (b64.isEmpty) continue;
    var mime = outputMime;
    if (b64.startsWith('data:')) {
      mime = mimeFromDataUrl(b64);
      final comma = b64.indexOf(',');
      if (comma >= 0) b64 = b64.substring(comma + 1);
    }
    final path = await AppDirectories.saveBase64Image(mime, b64);
    ...
    images.add((uri: uri, mimeType: mime));
  }
  return images;
}
