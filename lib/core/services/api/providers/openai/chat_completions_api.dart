  ...
  final cleaned = tools.map((tool) {
    ...
    if (fn is Map) {
      ...
      if (params is Map) {
        ...
        // agnes 等严格 OpenAI 兼容网关把 function.parameters 判为必填，
        // 缺字段直接 400（tools[i].function: missing field `parameters`）。
        // 这里补最小合法 JSON Schema，保证每个工具都能过反序列化。
        fnMap['parameters'] = <String, dynamic>{'type': 'object'};
      }
...
List<EmitToolCall> openaiCallsFromCompletionMessage(Map<String, dynamic>? msg) {
  ...
  for (var i = 0; i < tcs.length; i++) {
    ...
    try {
      args = parseToolCallArgs(f['arguments']);
    } catch (_) {
      args = <String, dynamic>{'__rawArguments': (f['arguments'] ?? '').toString()};
    }
    calls.add(
      emitToolCall(
        id: id,
...
Stream<StreamChunk> runOpenAIChatCompletionsToolFollowUps({
  ...
  // 动态工具扩容：每轮 follow-up 前重算工具声明（流程图① Tier 追加用）；
  // null 时使用固定的 [tools]。
  List<Map<String, dynamic>>? Function()? toolsOf,
  ...
  // 工具集每轮可重算（route_task 返回轨道后追加 Tier2/Tier3）。
  List<Map<String, dynamic>>? roundTools() => toolsOf?.call() ?? tools;
  ...
  yield* runClientToolFollowUps(
    ...
    sendFollowUp: () async* {
      final body2 = <String, dynamic>{
        ...
        if (roundTools() != null && roundTools()!.isNotEmpty)
          'tools': cleanToolsForCompatibility(roundTools() ?? const []),
      };
      ...
      final followupHeaders = customHeaders(
        config,
        modelId,
      ...
      final reqBodyJson = jsonEncode(body2);
      final http.StreamedResponse resp2;
      try {
        // 失败重连：瞬态网络故障与网关 5xx/429 在流未消费前退避重试
        resp2 = await HttpSendRetry.send(
          client,
          () => http.Request('POST', url)
            ..headers.addAll(followupHeaders)
            ..body = reqBodyJson,
        );
...
Stream<StreamChunk> runOpenAIChatCompletionsNonStreamToolFollowUps({
  ...
  yield* runClientToolFollowUps(
    ...
    sendFollowUp: () async* {
      ...
      final followBody = jsonEncode(reqBody);
      req.body = followBody;
      final resp2 = await HttpSendRetry.send(
        client,
        () => http.Request('POST', req.url)
