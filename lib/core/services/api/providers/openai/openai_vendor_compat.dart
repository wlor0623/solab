...
void applyCompatibleResponsesReasoning(
  ...
  if (isDeepSeek) {
    if (!isReasoning || isOff(thinkingBudget)) {
      // 关闭或模型漏标 reasoning 能力：显式 effort=none（DeepSeek 默认跟随上游）
      body['reasoning'] = {'effort': 'none'};
    } else {
      // 开启：保留调用方设置的官方 effort（含 max）
    }
    return;
  }
  ...
  body.remove('reasoning');
  // 关闭或模型漏标 reasoning 能力：显式关思考（Qwen3 流式默认开启）
  body['enable_thinking'] = isReasoning && !isOff(thinkingBudget);
  if (!(isReasoning && !isOff(thinkingBudget))) return;
...
void normalizeMoonshotKimiChatBody(
  ...
  body.remove('reasoning_effort');
  final userWantsOff = !isReasoning || isOff(thinkingBudget);
  if (_isKimiK25Model(upstreamModelId)) {
    // K2.5 服务端默认开启思考：显式设置而非删除参数
    body['thinking'] = {'type': userWantsOff ? 'disabled' : 'enabled'};
    _removeMoonshotKimiUnsupportedSamplingParams(body);
    return;
  }
  if (_isKimiOmitsSamplingParamsModel(upstreamModelId)) {
    // k2.7-code 等不支持 thinking 参数（非思考模型），只剥采样参数
    _removeMoonshotKimiUnsupportedSamplingParams(body);
    return;
  }
...
void applyVendorReasoningKnobs(
  ...
  if (info.isOpenRouter) {
    if (isReasoning) {
      ...
      // 关闭或模型漏标 reasoning 能力：显式禁用（OpenRouter 默认跟随上游）
      body['reasoning'] = {'enabled': false};
      body.remove('reasoning_effort');
    }
  } else if (info.isHy3) {
    // 修复"关了还有思考"：旋钮命中 vendor 后必须显式设置，禁止回退删除
    // （模型列表漏标 reasoning 能力时，删除=服务端默认开）。
    body['thinking'] = {
      'type': (off || !isReasoning) ? 'disabled' : 'enabled',
    };
    if (off || !isReasoning) body.remove('reasoning_effort');
  } else if (info.isAgnes) {
    final raw = body['chat_template_kwargs'];
    final options = raw is Map
        ? Map<String, dynamic>.from(raw)
        : <String, dynamic>{};
    options['enable_thinking'] = isReasoning && !off;
    body['chat_template_kwargs'] = options;
    body.remove('reasoning_effort');
  } else if (info.isLaguna) {
    body['chat_template_kwargs'] = {
      'enable_thinking': isReasoning && !off,
    };
    body.remove('reasoning_effort');
  } else if (info.isDashScope) {
    body['enable_thinking'] = isReasoning && !off;
    if (isReasoning && !off && thinkingBudget != null && thinkingBudget > 0) {
    ...
    body['thinking'] = {
      'type': (isReasoning && !off) ? 'enabled' : 'disabled',
    };
    body.remove('reasoning_effort');
  } else if (info.isVolc) {
    body['thinking'] = {
    ...
    body['thinking_mode'] = isReasoning && !off;
    body.remove('reasoning_effort');
    ...
    if (!(isReasoning && !off)) body.remove('reasoning_effort');
  }
}
