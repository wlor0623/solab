...
List<Map<String, dynamic>> toResponsesToolsFormat(
  ...
  return tools.map((tool) {
    ...
    // 严格网关把 parameters 判为必填；缺失时补最小合法 schema。
    out['parameters'] = params is Map<String, dynamic>
        ? params
...
List<EmitToolCall> responsesCallsFromIndexMap(
  ...
  for (final idx in sorted) {
    ...
    try {
      args = parseToolCallArgs(m['args']);
    } catch (_) {
      args = <String, dynamic>{'__rawArguments': (m['args'] ?? '').toString()};
    }
