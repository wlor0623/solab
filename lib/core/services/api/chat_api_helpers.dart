...
/// 工具调用参数解析。模型输出的 arguments 偶为非严格 JSON（``` 围栏、
/// 尾逗号、缺引号键等）。旧实现失败时静默置 {}，写操作因此在零参数下走
/// 默认分支（曾造成 apk_rebuild 盲目 decode 覆盖手工修改、so_patch_into_apk
/// 确认死循环）。现在先尽力修复；仍失败则返回 {"__rawArguments": raw}，
/// 由调度层统一拒绝执行并提示模型重发合法 JSON。
Map<String, dynamic> parseToolCallArgs(Object? rawArgs) {
  var str = (rawArgs ?? '').toString().trim();
  if (str.isEmpty) return <String, dynamic>{};
  final direct = tryParseArgsObject(str);
  if (direct != null) return direct;
  // 剥 ```/```json 围栏后重试
  final fence = RegExp(r'^```[a-zA-Z]*\s*([\s\S]*?)\s*```$').firstMatch(str);
  if (fence != null) {
    final inner = tryParseArgsObject(fence.group(1)!);
    if (inner != null) return inner;
    str = fence.group(1)!.trim();
  }
  // 尾逗号修复：} 或 ] 前的多余逗号
  final repaired = str.replaceAllMapped(RegExp(r',\s*([}\]])'), (m) => m.group(1)!);
  if (!identical(repaired, str)) {
    final fixed = tryParseArgsObject(repaired);
    if (fixed != null) return fixed;
  }
  return <String, dynamic>{'__rawArguments': str};
}

Map<String, dynamic>? tryParseArgsObject(String str) {
  try {
