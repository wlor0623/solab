...
class SolabFetchRequestPayload {
  ...
  static SolabFetchRequestPayload parse(Object? args) {
    if (args is! Map) {
      throw ArgumentError(
    ...
    return SolabFetchRequestPayload(
      url: uri,
      method: method,
      body: body,
      headers: headers,
...
class SolabFetcher {
  ...
  static Future<http.Response> _fetch(SolabFetchRequestPayload payload) async {
    try {
  ...
  static Future<Map<String, dynamic>> fetch(
    SolabFetchRequestPayload payload,
  ) async {
    try {
  ...
  static String _decodeBody(http.Response resp, {required String contentType}) {
    if (SolabFetchRequestPayload._charsetOf(contentType) != null) {
      return resp.body;
    }
    try {
  ...
  static String _bounded(String text, SolabFetchRequestPayload payload) {
...
class SolabFetchMcpServerEngine {
  bool _closed = false;
    ...
    try {
      ...
      switch (method) {
        ...
        case mcp.McpProtocol.methodCallTool:
          ...
          SolabFetchRequestPayload payload;
          try {
            payload = SolabFetchRequestPayload.parse(arguments);
          } catch (e) {
            return _ok(id, result: SolabFetcher._err(e.toString()));
          }

          if (name == 'kelivo_fetch' || name == 'solab_fetch') {
            return _ok(id, result: await SolabFetcher.fetch(payload));
          }
    ...
    Map<String, dynamic> schema() => {
      ...
      'properties': {
        ...
        'max_length': {
          ...
          'default': SolabFetchRequestPayload.defaultMaxLength,
          'minimum': 1,
          'maximum': SolabFetchRequestPayload.maximumMaxLength,
        },
        'start_index': {
          'type': 'integer',
          'description':
...
class SolabInMemoryClientTransport implements mcp.ClientTransport {
  final SolabFetchMcpServerEngine _server;
  final _messageController = StreamController<dynamic>.broadcast();
  final _closeCompleter = Completer<void>();
  bool _closed = false;

  SolabInMemoryClientTransport(this._server);
