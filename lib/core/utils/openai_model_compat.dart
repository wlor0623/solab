...
// Meta Muse Spark (1.1 / 1.2 / 1.2-contributor): Meta's own API accepts the
// minimal..xhigh effort dial, but common relays (LiteLLM before its 2026-08
// registry update, new-api gateways) reject reasoning_effort for this family
// with 400 UnsupportedParamsError — and tool-call follow-ups die silently
// for the user. Omitting the parameter is always accepted (Meta falls back
// to its default tier), so we never send it for Muse Spark.
...
String openAINormalizeReasoningEffort(String effort, String modelId) {
  ...
  if (normalizedEffort == 'off' || normalizedEffort == 'none') {
    ...
    // Models without a "none" tier (e.g. Muse Spark) fall to their closest
    // off-equivalent via offFallback; 'none' previously only fell through
    // the switch ladder.
