...
import '../services/mcp_server/mcp_http_server.dart';
...
class SettingsProvider extends ChangeNotifier {
  ...
  static const String _memoryPromptContractVersionKey =
      'memory_prompt_contract_version_v1';
  ...
  // MCP server (expose built-in tools over LAN as an MCP endpoint)
  static const String _mcpServerEnabledKey = 'mcp_server_enabled_v1';
  static const String _mcpServerPortKey = 'mcp_server_port_v1';
  static const String _mcpServerTokenKey = 'mcp_server_token_v2';
  ...
  Future<void> _load() async {
    ...
    _thinkingBudget = prefs.getInt(_thinkingBudgetKey) ?? -1;
    ...
    // v1-v2 used an obsolete injection mechanism and must be replaced. v3-v4
    // only compacts equivalent wording, so preserve user-edited prompts and
    // migrate each language only when it still equals the v3 default.
    final memoryContractVersion =
        prefs.getInt(_memoryPromptContractVersionKey) ?? 0;
    if (memoryContractVersion < MemoryPrompts.contractVersion) {
      final storedZh = prefs.getString(_memoryRulesPromptZhKey) ?? '';
      final storedEn = prefs.getString(_memoryRulesPromptEnKey) ?? '';
      final preserveCustom = memoryContractVersion >= 3;
      final storedZhHash = sha256.convert(utf8.encode(storedZh)).toString();
      final storedEnHash = sha256.convert(utf8.encode(storedEn)).toString();
      _memoryRulesPromptZh =
          preserveCustom &&
              storedZh.trim().isNotEmpty &&
              storedZhHash != MemoryPrompts.rulesV3ZhHash
          ? storedZh
          : MemoryPrompts.rulesZh;
      _memoryRulesPromptEn =
          preserveCustom &&
              storedEn.trim().isNotEmpty &&
              storedEnHash != MemoryPrompts.rulesV3EnHash
          ? storedEn
          : MemoryPrompts.rulesEn;
      unawaited(() async {
        await prefs.setString(_memoryRulesPromptZhKey, _memoryRulesPromptZh);
        await prefs.setString(_memoryRulesPromptEnKey, _memoryRulesPromptEn);
        await prefs.setInt(
          _memoryPromptContractVersionKey,
          MemoryPrompts.contractVersion,
        );
      }());
    } else {
    ...
    // MCP server (LAN tool endpoint)
    try {
      _mcpServerEnabled = prefs.getBool(_mcpServerEnabledKey) ?? false;
      _mcpServerPort =
          prefs.getInt(_mcpServerPortKey) ?? McpHttpServer.defaultPort;
      _mcpServerToken = prefs.getString(_mcpServerTokenKey) ?? '';
    } catch (_) {
      _mcpServerEnabled = false;
      _mcpServerPort = McpHttpServer.defaultPort;
      _mcpServerToken = '';
    }
    ...
    if (_providerConfigs.isEmpty) {
      ...
      ensureProviderConfig('SoLabIN', defaultName: 'SoLabIN');
  ...
  int _mcpServerPort = McpHttpServer.defaultPort;
  ...
  bool get mcpServerAuthEnabled => _mcpServerToken.isNotEmpty;

  Future<void> setMcpServerEnabled(bool v) async {
    if (_mcpServerEnabled == v) return;
    _mcpServerEnabled = v;
    notifyListeners();
    await _preferences.setBool(_mcpServerEnabledKey, v);
  }

  /// 访问保护开关：默认关闭（无 token，局域网直连）；
  /// 开启时生成随机 token，关闭时清空。
  Future<void> setMcpServerAuthEnabled(bool v) async {
    final next = v
        ? (_mcpServerToken.isEmpty ? _generateMcpToken() : _mcpServerToken)
        : '';
    if (_mcpServerToken == next) return;
    _mcpServerToken = next;
    notifyListeners();
    await _preferences.setString(_mcpServerTokenKey, next);
  }

  Future<void> setMcpServerPort(int port) async {
    final p = port.clamp(1024, 65535);
    if (_mcpServerPort == p) return;
    _mcpServerPort = p;
    notifyListeners();
    await _preferences.setInt(_mcpServerPortKey, p);
  }

  Future<void> regenerateMcpServerToken() async {
    _mcpServerToken = _generateMcpToken();
    notifyListeners();
    await _preferences.setString(_mcpServerTokenKey, _mcpServerToken);
  }

  static String _generateMcpToken() {
...
{content}
  ...
  int? _thinkingBudget =
      -1; // null = inherit; -1 = auto; 0 = off; >0 = budget tokens
...
class ProviderConfig {
  static const _solabInPublicApiKey = 'kelivo';
  ...
  static String _apiKeyFromJson(Map<String, dynamic> json) {
    ...
    return normalized == 'solabin' || normalized == 'kelivoin'
        ? _solabInPublicApiKey
        : '';
  }
  ...
  static String _defaultBase(String key) {
    ...
    if (k.contains('solabin') || k.contains('kelivoin')) {
      return 'https://text.pollinations.ai/openai';
    }
    if (k.contains('opencode')) return 'https://opencode.ai/zen/v1';
  ...
  static ProviderConfig defaultsFor(String key, {String? displayName}) {
    ...
    switch (kind) {
      ...
      case ProviderKind.openai:
        // Special-case SoLabIN default models and overrides
        if (lowerKey.contains('solabin') || lowerKey.contains('kelivoin')) {
          return ProviderConfig(
            id: key,
            enabled: defaultEnabled(key),
            name: displayName ?? key,
            apiKey: _solabInPublicApiKey,
            baseUrl: _defaultBase(key),
