...
class McpProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
          ...
          server.copyWith(
            ...
            tools: <McpToolConfig>[
              for (final tool in server.tools)
                tool.name == 'solab_fetch'
                    ? tool.copyWith(name: 'kelivo_fetch')
                    : tool,
            ],
          )
        else
    ...
    if (jsonEncode(normalizedServers.map((e) => e.toJson()).toList()) !=
        jsonEncode(_servers.map((e) => e.toJson()).toList())) {
      await _persistServers(normalizedServers);
      _servers = normalizedServers;
    }
    ...
    try {
      ...
      if (server.transport == McpTransportType.inmemory) {
        ...
        await client.connect(
          SolabInMemoryClientTransport(SolabFetchMcpServerEngine()),
        );
      } else {
