...
import 'package:solab/secrets/fallback.dart';
...
class ModelRegistry {
  ...
  // every Qwen 3.7 Max id is multimodal. Same for GLM-5: glm-5.3-flash is the
  // series' first natively multimodal model (2026-08-26, the former ox-alpha);
  // glm-5 / 5.1 / 5.2 / 5.3 flagships are text-only, so only the flash id gets
  // image input.
  static final RegExp vision = RegExp(
    // GPT family incl. 4o, 4.1, 5 (exclude gpt-5-chat), and OpenAI o* series
    r'(gpt-4o|gpt-4\.1|gpt-5(?!-chat)|o\d|gemini|claude|kimi-k-?2([-.])(?:5|6|7)|kimi-k3(?:$|[/_:@.-])|muse-spark-1\.[12](?:$|[/_:@.-])|doubao.+(?:1([-.])(?:6|8)|seed-2|seed-evolving)|grok-4|step-3|intern-s1|internvl|minimax-m3(?:$|[/_:@.-])|mimo-v2(?:-omni(?:$|[/_:@])|\.5(?:$|[/_:@]))|agnes-(?:1\.5-flash|2\.0-flash|2\.5-(?:flash|pro(?:-alpha)?))|ox-alpha|hunyuan[^/]*vision|sensenova-6\.7-flash-lite|deepseek.+(?:vision|[-_]vl)|qwen.+[-_]vl|qvq|glm-[\w.-]*v(?:ision)?|glm-5\.3-flash(?:$|[/_:@.-])|llama-4|llama-3\.2[^/]*vision|pixtral|phi-4[^/]*multimodal|gemma[-_]?3)',
    caseSensitive: false,
  );
  // Tool-using models
  static final RegExp tool = RegExp(
    (r'(gpt-4o|gpt-4\.1|gpt-oss|gpt-5(?!-chat)|o\d|'
            r'gemini|claude|'
            r'qwen-?3|doubao.+(?:1([-.])(?:6|8)|seed-2|seed-evolving)|grok-4|kimi-k-?2|'
            r'kimi-k3(?:$|[/_:@.-])|muse-spark-1\.[12](?:$|[/_:@.-])|'
            r'step-3|intern-s1|glm-4([-.])(?:5|6|7)|glm-5|minimax-m-?[23]|'
            r'deepseek-(?:r1|v3|chat|v3\.1|v3\.2|v4)|'
            r'deepseek-reasoner|'
            r'mimo-v2|'
            r'agnes-(?:2\.0-flash|2\.5-(?:flash|pro(?:-alpha)?))|ox-alpha|'
            r'tc-code-latest|hy3(?:-preview|-\d{6})?(?:$|[/_:@.-])|hunyuan-(?:turbos|functioncall|funcall)|'
  ...
  static final RegExp reasoning = RegExp(
    (r'(gpt-oss|gpt-5(?!-chat)|o\d|'
            ...
            r'mimo-v2|'
            r'agnes-(?:2\.0-flash|2\.5-(?:flash|pro(?:-alpha)?))|ox-alpha|'
            r'tc-code-latest|hy3(?:-preview|-\d{6})?(?:$|[/_:@.-])|'
            r'laguna'
            r')')
        .replaceAll(' ', ''),
    caseSensitive: false,
  ...
  static bool _isQwenVisionModel(String id) {
    ...
    if (RegExp(r'qwen-?3([-.])8(?:$|[-.])').hasMatch(lower)) return true;
  ...
  static ModelInfo fromApiEntry(
    ...
    final output = <Modality>[Modality.text];
    final abilities = <ModelAbility>[];
    final architecture = entry['architecture'];
    final architectureMap = architecture is Map ? architecture : const {};
    final inputNames = _stringSet(
      architectureMap['input_modalities'] ??
          entry['input_modalities'] ??
          entry['inputModalities'],
    );
    final outputNames = _stringSet(
      architectureMap['output_modalities'] ??
          entry['output_modalities'] ??
          entry['outputModalities'],
    );
    if (inputNames.any((value) => value.contains('image'))) {
      input.add(Modality.image);
    }
    if (outputNames.any((value) => value.contains('image'))) {
      output.add(Modality.image);
    }

    final parameters = _stringSet(
      entry['supported_parameters'] ?? entry['supportedParameters'],
    );
    if (parameters.any(
      (value) =>
          value == 'tools' ||
    ...
    if (parameters.any((value) => value.contains('reasoning'))) {
      abilities.add(ModelAbility.reasoning);
    }

    final capabilities = entry['capabilities'];
    if (capabilities is Map) {
      bool enabled(String key) => _capabilityEnabled(capabilities[key]);
      if (enabled('tool_use') ||
          enabled('tools') ||
          enabled('function_calling')) {
        abilities.add(ModelAbility.tool);
      }
      if (enabled('reasoning') ||
          enabled('extended_thinking') ||
          enabled('adaptive_thinking')) {
        abilities.add(ModelAbility.reasoning);
      }
      if (enabled('vision') || enabled('image_input')) {
        input.add(Modality.image);
      }
    }
  ...
  static bool _capabilityEnabled(Object? value) {
    ...
    if (value is Map) {
      return _capabilityEnabled(value['supported']) ||
          _capabilityEnabled(value['enabled']);
    }
    return false;
  }
}
  ...
  Future<List<ModelInfo>> listModels(ProviderConfig cfg) async {
    ...
    try {
      ...
      for (var page = 0; page < 100; page++) {
        final uri = Uri.parse('${cfg.baseUrl}/models').replace(
        ...
        if (obj['has_more'] != true || data.isEmpty) break;
        final next = obj['last_id']?.toString().trim();
        if (next == null || next.isEmpty || next == afterId) break;
        afterId = next;
      }
      return out;
    } finally {
    ...
    try {
      ...
      try {
        ...
        for (var page = 0; page < 100; page++) {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) break;
          final obj = jsonDecode(res.body) as Map<String, dynamic>;
          final arr = (obj['models'] as List?) ?? const [];
          for (final e in arr) {
            if (e is Map) {
          ...
          final next = obj['nextPageToken']?.toString().trim();
