...
class HomePageController extends ChangeNotifier {
  ...
  String _localizeGenerationError(AppLocalizations l10n, String error) {
    switch (error) {
      ...
      default:
        return '${l10n.generationInterrupted}: ${_friendlyErrorText(error)}';
    }
  }

  /// 把底层异常长串压缩成一句用户能读懂的话。
  static String _friendlyErrorText(String error) {
    final lower = error.toLowerCase();
    if (lower.contains('401') || lower.contains('unauthorized')) {
      return '密钥无效或未授权，请检查平台 API Key';
    }
    if (lower.contains('402') || lower.contains('insufficient')) {
      return '账户余额不足';
    }
    ...
    if (lower.contains('provider error')) {
      return '平台返回错误，请稍后重试或更换模型';
    }
    if (lower.contains('response incomplete')) {
      return '响应中断，请重试';
    }
    // 兜底：截断过长的异常串，保留首行
