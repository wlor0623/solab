...
class ChatActions {
  ...
  static Future<void> cancelAllActiveGenerations() async {
    ...
    final conversationIds = <String>{
      ...actions.chatService.getAllConversations().map((item) => item.id),
      ...actions._conversationStreams.keys,
      ...actions._loadingConversationIds,
      ...actions._sendInFlightClaims.keys,
      ...actions._cancelStreamingFutures.keys,
    };
    for (final id in conversationIds) {
      if (actions._hasActiveGeneration(id) ||
          actions._sendInFlightClaims.containsKey(id)) {
        await actions.cancelStreamingById(id);
      }
    }
  }
  ...
  ChatMessage _messageWithCurrentReasoning(ChatMessage message) {
    ...
    final reasoningSegmentsJson = segments != null || details != null
        ? streamController.serializeSegmentsWithCurrentSplits(
            messageId,
            segments ?? const [],
  ...
  void _clearGenerationRuntimeState(ChatMessage message) {
    ...
    // Android 保活释放由外层入口（sendMessage/regenerate/tool-continue）
    // 的 finally 配对执行，这里不再重复 end（防止双重递减）。
  }

  /// 用户是否要求前台服务常驻（设置「后台聊天」on/onNotify）。
  bool _isPersistentBackgroundPreferred() {
    try {
      return contextProvider
              .read<SettingsProvider>()
              .androidBackgroundChatMode !=
  ...
  static List<MessagePart> assistantPartsForStreamError({
    ...
    if (partialContent.isEmpty) {
      // 取消/中断类错误不写错误文本：保留已积累的思考与工具卡片即可。
      if (displayContent.isEmpty) return parts;
  ...
  /// 取消/中断类错误（用户停止、工具打断、连接被主动断开）：
  /// 属于正常停止流程，不应弹"生成已中断"错误提示。
  @visibleForTesting
  static bool isCancellationLikeError(Object error) {
  ...
  Future<ChatActionResult> _sendMessageClaimed({
    ...
    if (settings.mcpServerEnabled) {
      return ChatActionResult.error('mcp_mode_active');
    }
  ...
  Future<void> _runSendGeneration({
    ...
    try {
      ...
      // 上下文压缩（有时限）：已生成的会话摘要替换旧消息为一条摘要，
      // 近期未总结消息完整保留——上下文满了压缩旧内容而非硬裁失忆。
      final contextMessages = _applyConversationSummary(
      ...
      // Android 生成期保活：整个任务（含工具多轮）持有一次，外层配对释放。
      // 注意：send 已提前 return，这里跑在 unawaited 的 _runSendGeneration 内。
      await AndroidBackgroundManager.beginGenerationHold();
      try {
        await _executeGeneration(ctx);
      } finally {
        await AndroidBackgroundManager.endGenerationHold(
          userWantsPersistent: _isPersistentBackgroundPreferred(),
        );
      }
    } catch (e) {
  ...
  /// 上下文压缩（有时限）：已生成的会话摘要（覆盖前 lastSummarizedMessageCount
  /// 条旧消息）替换旧消息为一条「历史摘要」，近期未总结消息完整保留——
  /// 上下文满了时压缩旧内容继续追加，而不是硬裁导致 AI 失忆。
  /// 摘要为空/未生成时不改动（保持全量历史）。
  static List<ChatMessage> _applyConversationSummary(
    List<ChatMessage> messages, {
    required Conversation conversation,
  }) {
    final summary = conversation.summary;
    if (summary == null || summary.trim().isEmpty) return messages;
    final summarizedCount = conversation.lastSummarizedMessageCount;
    if (summarizedCount <= 0 || messages.isEmpty) return messages;
    // 已总结条数可能超过当前消息数（历史清理后）——至少保留最后一条历史
    final replaceCount = summarizedCount < messages.length
        ? summarizedCount
        : messages.length - 1;
    if (replaceCount <= 0) return messages;
    final summaryMessage = ChatMessage(
      id: 'context-summary-${conversation.id}',
      role: 'user',
      content: '【历史会话摘要（前 $replaceCount 条消息的要点，非当前对话原文）】\n$summary',
      conversationId: conversation.id,
    );
    return <ChatMessage>[summaryMessage, ...messages.sublist(replaceCount)];
  }
  ...
  static Future<int> resolveContextReadLimit({
    required Assistant? assistant,
    bool forceCompleteContext = false,
    required Future<int> Function() resolvePersistedCount,
  }) async {
    if (!forceCompleteContext &&
        (assistant?.limitContextMessages ?? false) &&
    ...
    if (forceCompleteContext) {
      return count > 0 ? count : Assistant.maxContextMessageSize;
    }
  ...
  Future<void> _cancelStreamingByIdOnce(String cid) async {
    // 打断引擎长任务（DEX/SO 分析、jadx 等）：Kotlin 检查点抛
    // CancellationException → TASK_CANCELLED，工具链立即停止。
    // unawaited：不引入 async gap（保持取消时序，避免竞态测试受影响）
    unawaited(ApkToolchainService.cancelTask().catchError((_) => false));
  ...
  Future<void> _executeGeneration(stream_ctrl.GenerationContext ctx) async {
    ...
    _streamingToolEvents[state.messageId] = chatService
        ...
        .map((event) => Map<String, dynamic>.from(event))
        .toList();
    // 网络重试判定基线：本轮开始前已存在的工具事件数
    state.toolEventRoundBaseline =
        _streamingToolEvents[state.messageId]?.length ?? 0;
  ...
  Future<void> _finishStreaming(
    ...
    try {
      ...
      // Android 保活释放由外层入口 finally 配对，此处不重复 end。
      if (chatController.publishTerminalMessage(finalizedMessage)) {
        onMessagesChanged?.call();
      }
  ...
  Future<void> _handleStreamError(
    ...
    final errorParts = await _sanitizeAssistantImageParts(
      assistantPartsForStreamError(
        parts: _assistantPartsForState(state),
        partialContent: partialContent,
        errorText: isCancellationLikeError(e) ? '' : errorText,
      ),
    );
    ...
    try {
      ...
      _conversationStreams.remove(conversationId);
      // 取消/中断类错误不弹"生成已中断"提示：正常停止不该被当成错误。
