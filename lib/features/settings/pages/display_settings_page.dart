...
class _DisplaySettingsPageState extends State<DisplaySettingsPage> {
  ...
  Future<void> _showAndroidBackgroundChatSheet(BuildContext context) async {
    ...
    // 保活修复：setEnabled 失败不再静默——提示用户保活未生效及原因
    void showEnableFailure() {
      final reason = AndroidBackgroundManager.lastEnableError ?? '未知原因';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('保活未生效：$reason（可尝试在系统设置中允许本应用后台运行/忽略电池优化）')),
      );
    }
    ...
    switch (choice) {
      ...
      case 'on':
        ...
        try {
          ...
          final ok = await AndroidBackgroundManager.setEnabled(
            true,
            networkRequired: false,
          );
          if (!ok) showEnableFailure();
          await NotificationService.ensureInitialized();
          final notificationsGranted =
              await NotificationService.ensureAndroidNotificationsPermission();
          // 授权后（或已授权）重发前台通知，确保 FGS 通知立即可见。
          if (notificationsGranted) {
            await AndroidBackgroundManager.refreshNotification();
          }
          // 国产 ROM 冻结进程的主要防线：引导用户豁免电池优化
          final ignoring =
      ...
      default:
        ...
        // MCP 模式在跑时 FGS 由它接管，关后台聊天不撤（防互杀）
        if (!McpHttpServer.instance.isRunning) {
          try {
